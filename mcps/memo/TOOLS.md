...
- **カテゴリ**: 省略すると `OTHERS` に分類される。カテゴリ名は大文字に正規化して保存される (`work`→`WORK`)。詳細は後述の「カテゴリ」。
...
- **カテゴリ**: `category` を省略すると変更しない。空文字を渡すと `OTHERS` に戻る。指定した値は大文字に正規化される。
...
すべてのメモは 1 つの**カテゴリ**に属する。カテゴリを指定せずに作ったメモ・既存メモは既定の `OTHERS` に分類される。

- **正規化**: カテゴリ名は前後の空白を除いて**大文字化**して保存・照合する。`work` / `Work` / `WORK` は同じカテゴリになる。
- **指定 (書き込み)**: `create_memo` / `update_memo` の `category` で付与・変更する。`update_memo` で `category` を省略すると変更しない。
...
- **カテゴリ一覧の返却**: 成功時のメッセージに、切り替え先ユーザーのメモが持つ**カテゴリ一覧**を併記する (例: `Switched user to 'alice'. メモのカテゴリ: PRIVATE, WORK`)。以降の `list_memos` / `search_memos` / `semantic_search_memos` を `category` で絞り込む際の手掛かりになる。メモが無ければ「カテゴリを持つメモはまだありません」と返す。
