...
def test_ranks_by_similarity_descending(fake_embed):
    a = create_memo_db(ALICE, "ペットの話", "犬と猫のペット")
    b = create_memo_db(ALICE, "投資の話", "株式投資の話")
...
def test_respects_limit(fake_embed):
    for i in range(3):
        create_memo_db(ALICE, f"m{i}", "犬と猫のペット")
    results = service.semantic_search(ALICE, "ペットについて", limit=2)
    assert len(results) == 2


def test_skips_empty_summary(fake_embed):
    create_memo_db(ALICE, "空概要", "")
    create_memo_db(ALICE, "有概要", "犬と猫のペット")
    results = service.semantic_search(ALICE, "ペットについて")
    assert {m["title"] for m in results} == {"有概要"}


def test_cache_avoids_recomputing_memo_embeddings(fake_embed):
    create_memo_db(ALICE, "m", "犬と猫のペット")
    service.semantic_search(ALICE, "ペットについて")
    service.semantic_search(ALICE, "ペットについて")
...
def test_summary_change_triggers_one_recompute(fake_embed):
    m = create_memo_db(ALICE, "m", "犬と猫のペット")
    service.semantic_search(ALICE, "ペットについて")
    update_memo_db(ALICE, m["id"], summary="株式投資の話")
    service.semantic_search(ALICE, "ペットについて")
