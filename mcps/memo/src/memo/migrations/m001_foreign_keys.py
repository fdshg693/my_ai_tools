...
def _create_name_triggers(db: sqlite3.Connection) -> None:
    """この移行時点の (``memos.user`` = ユーザー名 を前提とした) トリガーを作る。

    ``infra.database._create_triggers`` は後続の m002 で ``user_id`` ベースへ
    変わったため、この歴史的移行の整合性を保つよう当時の定義をここに凍結する
    (m002 が後でこれを drop して ``user_id`` 版へ作り替える)。
    """
    db.execute("""
