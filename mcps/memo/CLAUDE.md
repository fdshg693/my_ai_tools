...
| `server/web/static/admin.html` / `admin.css` / `admin.js` | The admin UI (vanilla HTML/CSS/JS, no build step — same approach as `dynamic_prompt/static`). `admin.js` calls the `/api/users` REST endpoints; `name` is immutable so only `display_name`/`note` are editable, and the `admin` row has no delete button. Each user row has a **メモ** button that opens a read-only paginated memo panel (`GET /api/users/{name}/memos`) with 前へ/次へ controls and a page/total indicator (`PER_PAGE`=20). |
...
| `tests/test_admin_server.py` | Tests for the `memo-admin` REST layer via Starlette's `TestClient` (httpx): list/create/get/update/delete, name-required + duplicate (409) + missing (404) cases, partial update (only supplied fields), the `admin`-undeletable guard (403), HTML serving, and the per-user memo listing (`/api/users/{name}/memos`: empty, missing-user 404, user-scoping, pagination, `per_page` cap, invalid-param fallback). Reuses `conftest.py`'s temp DB. |
