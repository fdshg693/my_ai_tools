...
| GET | `/api/users/{name}/memos` | `count_memos_db` + `list_memos_db` (repository, read-only) | Paginated memo list for one user (newest-first). Query `page` (1-based) / `per_page` (≤100, default 20). Returns `{user, items, page, per_page, total, total_pages}`; 404 (`UserNotFound`) |
| POST | `/api/users/{name}/memos` | `create_memo_db(name, title, summary)` (repository) | Create a memo owned by `name`. `title` required → 400; user absent → 404; 201 on success |
| PUT | `/api/users/{name}/memos/{memo_id}` | `update_memo_db(name, memo_id, title, summary)` (repository, `is_admin=False`) | Partial update (omitted field unchanged); empty `title` → 400; memo not under that user → 404 |
