...
| `repository/memo.py` | Pure data access for memos (`*_memo_db`). get/list/search/update/delete take a trailing `is_admin`; when True the user filter is dropped to cover all memos (including orphans). `list_memos_db` also takes `offset` for paging; `count_memos_db(user, is_admin)` returns the total for the same filter (used by the web memo-list pager). `normalize_category()` (trim + uppercase, empty/`None` → `OTHERS`) is applied on every write so stored values are canonical; `create_memo_db`/`update_memo_db` take a `category`, and list/count/search take an optional `category` filter (built once via the private `_base_filter` helper shared by all three — `None` = all categories). `list_categories_db(user, is_admin)` returns the user's distinct categories (name-sorted; used by the `switch_user` tool). Uses only `_connect_db`; no authorization. |
...
| `tests/test_memo_repository.py` | Unit tests for `repository.memo` (CRUD + search + user isolation + admin privilege + category: `normalize_category` rules, default `OTHERS`, list/count/search filtering, update set/keep/reset, `list_categories_db` distinct/sorted/per-user/admin/empty). |
...
| `tests/test_mcp_client.py` | Integration tests through an MCP client (tool registration + authorization + admin cross-user + user management + semantic search + category filter roundtrip via `create_memo`/`list_memos`/`search_memos`; monkeypatches `service.memo.embed_text`. Runs via `asyncio.run()` without depending on pytest-asyncio). Also covers `switch_user` (stdio path, incl. the target-category list in its success message), the `current_user` HTTP `client_id` branches (monkeypatching `auth.get_http_request` — i.e. `memo.server.mcp.auth` — with a dummy request), and the audit log (`caplog` for the INFO tool line and the DEBUG `initialize` line; the logger name is `memo.server.mcp.logging_middleware`). |
...
| `category` | TEXT NOT NULL DEFAULT 'OTHERS' | Category. Normalized (trim + uppercase) on write; empty/unspecified → `OTHERS` (`OTHERS_CATEGORY`). See **Category** below |
